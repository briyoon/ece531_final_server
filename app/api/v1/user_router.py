...
from schemas import RegisterDevice
...
async def get_user_devices(user: Annotated[User, Depends(get_user_from_token)]):
...
async def get_device_reports(
    user: Annotated[User, Depends(get_user_from_token)], device_id: int
):
...
async def register_device(
    user: Annotated[User, Depends(get_user_from_token)], device: RegisterDevice
):
